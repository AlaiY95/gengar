...
pub async fn fetch_sushiswap_v2(
    ...
    info!(
        "Initiating SushiSwap V2 pool fetch: blocks {} to {}",
    ...
    for log_entry in event_logs.iter() {
        ...
        let mut new_pool = Pool {
            ...
            token0: token_a,
            token1: token_b,
            fee: 300, // SushiSwap V2 default fee
    ...
    info!(
        "SushiSwap V2 pool fetch complete. Total pools: {}",
        liquidity_pools.len()
    );
    Ok(liquidity_pools)
}
